...
/**
 * "Want your own?" interest capture for /create-your-digital-twin.
 * Client-side only, no real email send — hard rule in this workspace
 * (Claude never sends emails on Francisco's behalf, see
 * MEMORY.md "No emails rule"). This just appends an entry to a local JSON
 * file in the AI_Lab repo root so Francisco can see who's interested.
 *
 * process.cwd() for this Next.js app is projects/ai_operating_map_package,
 * not the AI_Lab repo root — resolve two directories up explicitly rather
 * than assuming.
 */
