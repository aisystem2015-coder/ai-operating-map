...
export interface VoxelTopographyGridProps {
  ...
  wireColor?: string;
  speed?: number;
  /** Deep background the isometric scene clears to each frame. */
  bgColor?: string;
  /** Full-bleed background mode (absolute inset-0, no demo frame). */
  fill?: boolean;
  className?: string;
}
...
export function VoxelTopographyGrid({
  tileSize = 28,
  maxHeight = 70,
  primaryColor = '#2563eb', // Francisco brand — signal blue (retired the #6366f1 indigo)
  wireColor = 'rgba(59, 130, 246, 0.4)',
  speed = 0.015,
  bgColor = '#060a17', // deep ink navy, matches the brand's dark ground
  fill = false,
  className = '',
  ...
  }, [tileSize, maxHeight, primaryColor, wireColor, speed, bgColor]);

  // Full-bleed background mode — fills whatever positioned parent it sits in.
  if (fill) {
    return (
      <div
        ref={containerRef}
  ...
  return (
    <div className={`w-full flex items-center justify-center p-4 sm:p-8 bg-slate-950 min-h-[500px] ${className}`}>
      {/* Container Frame */}
      <div className="relative w-full max-w-5xl aspect-video rounded-2xl p-2 bg-slate-900/80 border border-slate-800/80 shadow-[0_0_50px_-12px_rgba(37,99,235,0.25)] backdrop-blur-md">
        <div
          ref={containerRef}
